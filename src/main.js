...
const constructLayerToDna = (_dna = [], _layers = []) => {
	let mappedDnaToLayers = _layers.map((layer, index) => {
		let selectedElement = layer.elements.find((e) => e.id == cleanDna(_dna[index]));
		return {
			name: layer.name,
		...
		while (editionCount <= layerConfigurations[layerConfigIndex].growEditionSizeTo) {
			...
			if (isDnaUnique(dnaList, newDna)) {
				...
				await Promise.all(loadedElements).then((renderObjectArray) => {
					...
					console.log(`Created edition: ${abstractedIndexes[0]}, with DNA: ${sha1(newDna.join(""))}`);
				});
				dnaList.push(newDna);
				editionCount++;
